...
class CNNTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Reshape input for CNN (assuming input_dim represents sequence length)
        x = x.unsqueeze(1)
        x_corrupted = x_corrupted.unsqueeze(1)

        # Apply CNN
        x = self.cnn(x)
        x_corrupted = self.cnn(x_corrupted)

        # Flatten the output from CNN
        x = x.view(x.size(0), -1)
        x_corrupted = x_corrupted.view(x_corrupted.size(0), -1)

        # Continue with Transformer
        embeddings = self.encoder(x)
        attn_output = self.attention(embeddings)
        embeddings_with_attention = embeddings + attn_output
